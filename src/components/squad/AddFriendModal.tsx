    ...
    <div className="scrim" onClick={onClose} style={{ zIndex: 300 }}>
      <div className="modal" style={{ maxWidth: '500px' }} onClick={e => e.stopPropagation()}>
        <div className="modal-head" style={{ padding: '20px 24px' }}>
          <h3 style={{ fontSize: '18px' }}>{t('friends.modal_title')}</h3>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', maxHeight: '380px', overflowY: 'auto', padding: '2px' }}>
            {results.length > 0 ? results.map(p => {
              ...
              return (
                <div key={p.id} className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px', border: '1px solid var(--line-2)', background: 'var(--card)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
                    <UserAvatar profile={p} className="avatar md" style={{ width: '44px', height: '44px', background: '#6938E8', color: '#fff', fontSize: '15px', fontWeight: 800, flexShrink: 0 }} />
                    <div style={{ minWidth: 0 }}>
                      <div className="tx-name" style={{ fontSize: '15px', fontWeight: 700, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{p.display_name || t('friends.name_fallback')}</div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginTop: '2px' }}>
                        <span style={{ fontSize: '11px', color: 'var(--purple-600)', fontWeight: 600, background: 'var(--purple-50)', padding: '1px 6px', borderRadius: '4px' }}>
                          ID: {p.tieugon_id || t('friends.id_fallback')}
                        </span>
                        <span style={{ color: 'var(--t4)', fontSize: '12px' }}>•</span>
                        <span className="tx-note" style={{ fontSize: '11px', color: 'var(--t3)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '120px' }}>{p.email || t('friends.email_fallback')}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ flexShrink: 0, marginLeft: '12px' }}>
                    ...
                    {status === 'received' && (
                      <button className="btn btn-primary btn-sm" style={{ padding: '6px 12px' }} onClick={() => alert(t('friends.accept_notice'))}>
              ...
              <div style={{ textAlign: 'center', padding: '48px 20px', background: 'var(--bg-2)', borderRadius: '16px' }}>
                ...
                <p style={{ color: 'var(--t2)', fontWeight: 700, fontSize: '16px' }}>{t('friends.no_results')}</p>
                <p style={{ color: 'var(--t3)', fontSize: '13px', marginTop: '6px' }}>{t('friends.no_results_sub')}</p>
              </div>
            ) : !loading && (
              <div style={{ textAlign: 'center', padding: '48px 20px', border: '2px dashed var(--line-2)', borderRadius: '16px' }}>
                ...
                <p style={{ color: 'var(--t2)', fontWeight: 600 }}>{t('friends.start_connect')}</p>
                <p style={{ color: 'var(--t3)', fontSize: '12px', marginTop: '4px', maxWidth: '240px', margin: '4px auto 0' }}>{t('friends.start_connect_sub')}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="modal-foot" style={{ background: 'var(--bg-2)', padding: '16px 24px' }}>
          <button className="btn btn-ghost" style={{ width: '100%', fontWeight: 600 }} onClick={onClose}>{t('friends.close')}</button>
        </div>
      </div>
    </div>
  );
